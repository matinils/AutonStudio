...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not configWindowActive and event0 == '-CONFIG_BUTTON-':
            ...
            canvas.draw_text('18 in.', [162, 13], color='black')
        ...
        if not studioWindowActive and event0 == '-CONTINUE_BUTTON-':
            ...
            paths_tab = [[sg.Listbox(values=[], size=(50, 6), key='-PATH_LIST-')],
                         ...
                         [sg.Text('Velocity'), sg.InputText(enable_events=True, size=[10,1], key='-VELOCITY_INPUT-')],
        ...
        while True and studioWindowActive:  # Event Loop
            ...
            if simulating:
                ...
                for i in range(1, len(points)):
                    deltas = hf.calculate_movement_per_frame(points[i-1], points[i], inches_per_second=velocities[i-1], frames_per_second=60, pixels_per_inch=5)
                ...
                for t in turns:
                    if t[0] == i - 1:
                        if prevTurn is None:
                            ...
                            heading = prevTurn[1]
                    prevTurn = t
                pathStrings.append('Path #' + str(i) + ': ' + hf.generate_path_string(convertedPoints[i - 1], convertedPoints[i], velocities[i-1], heading))
